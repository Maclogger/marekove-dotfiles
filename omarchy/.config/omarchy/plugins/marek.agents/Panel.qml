...
Panel {
  ...
  WidgetButton {
    ...
    // The built-in label is a single centered Text, so it is switched off and
    // replaced by the mark + percent row below. implicitWidth follows that row.
    labelVisible: false
    hasVisualContent: root.barPercentText !== ""
    horizontalMargin: 8.75
    verticalPadding: 8.75
    fixedWidth: (root.bar && root.bar.vertical)
      ? -1
      : barContent.implicitWidth + button.scaledHorizontalMargin * 2
    active: root.alarming
    // The glyph alone can't say which window or when it resets, so the detail
    // that used to require opening the panel goes in the hover tooltip.
    tooltipText: {
      if (!root.provider) return ""
      var lines = [root.provider.name || "Agents"]
      var windows = root.limitWindows(root.provider)
      for (var i = 0; i < windows.length; i++) {
        var w = windows[i]
        lines.push(w.title + ": " + Math.round(w.percent * 100) + "%")
      }
      return lines.join("\n")
    }
    ...
    // The provider's own mark next to the number. It follows the selected
    // provider, so middle-clicking to Codex swaps the logo too. Same
    // candidate-walk as the panel hero: try the light twin on a light bar,
    // fall back to the plain mark, and if neither loads show only the percent.
    Row {
      id: barContent
      ...
      Item {
        id: barMark
        // Only the plain mark, not iconCandidatesForProvider()'s light/dark
        // walk: the bar is dark, claude.svg ships no -light twin, and probing
        // for the missing file logs a failed load on every shell start.
        property var candidates: root.provider
          ? [Qt.resolvedUrl("assets/" + root.provider.providerId + ".svg")] : []
        // Provider objects are rebuilt on every refresh, so compare content,
        // not array identity, or the fallback walk restarts forever.
        property string candidatesKey: candidates.join("\n")
        property int candidateIndex: 0
        onCandidatesKeyChanged: candidateIndex = 0

        readonly property real markSize: Math.round(button.fontSize * 1.15)
        width: barMarkImage.status === Image.Ready ? markSize : 0
        height: markSize
        ...
        Image {
          id: barMarkImage
          anchors.fill: parent
          source: barMark.candidateIndex < barMark.candidates.length
            ? barMark.candidates[barMark.candidateIndex] : ""
          sourceSize.width: barMark.markSize * 3
          sourceSize.height: barMark.markSize * 3
