...
Panel {
  ...
  function toggleDisplay(name, enabled) {
    ...
    // Upstream uses `hyprctl keyword`, which Hyprland rejects outright under
    // Omarchy 4's Lua parser ("keyword can't work with non-legacy parsers"),
    // making this row a no-op. `eval` is the supported equivalent.
    // Re-enabling goes through a reload so the display comes back with the
    // position and scale from monitors.lua rather than an auto-derived one.
    actionProc.command = enabled
      ? ["hyprctl", "eval", 'hl.monitor({ output = "' + name + '", disabled = true })']
      : ["hyprctl", "reload"]
    if (!actionProc.running) actionProc.running = true
  }

  // Upstream calls omarchy-hyprland-monitor-scaling, which applies the scale
  // with position = "auto" and so reorders a multi-monitor layout. This helper
  // keeps the current left-to-right order, reflows positions for the new
  // logical widths, and persists the result to ~/.config/hypr/monitors.lua.
  function setScale(scale) {
    actionProc.command = ["omarchy-monitor-scale-keep-layout", String(scale)]
    if (!actionProc.running) actionProc.running = true
  }
